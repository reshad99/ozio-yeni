            ...
            var initDatatable = function() {
                window.datatable = dt = $("#kt_datatable_example_1").DataTable({
                    ...
                    ajax: function(data, callback, settings) {
                        ...
                        data.filter = {};
                        if (window.searchBtnClicked && window.searchBtnClicked !=
                            undefined) {
                            console.log('filters added');
                            console.log(window.searchBtnClicked);
                            //add filters
                            data.filter.date_start = $('#filter_date_start').val();
                            data.filter.date_end = $('#filter_date_end').val();
                            data.filter.status = $('#filter_status').val();
                            data.filter.subscriber_number = $('#filter_subscriber_number')
                                .val();
                            data.filter.meter_number = $('#filter_meter_number').val();
                            data.filter.filter_phone_number = $('#filter_phone_number')
                                .val();
                            data.filter.filter_worker_id = $('#filter_worker_id').val();
                            data.filter.filter_custom_id = $('#filter_custom_id').val();
                            data.filter.filter_category_id = $('#filter_category_id').val();
                            data.filter.filter_sub_category_id = $(
                                    '#filter_sub_category_id')
                                .val();
                            data.filter.caller_name = $('#filter_caller_search').val();

                            window.searchBtnClicked = false;
                        }

                        //check all filters if have empty value remove from ajax request
                        removeNullValues(data.filter);



                        data.with= ['caller', 'category', 'subCategory', 'staff'];
                        // AJAX isteğini manuel olarak yap
                        $.ajax({
                            url: "{{ route('admin.ajax.emergencycall.datatable') }}", // AJAX isteğinin yapılacağı URL
                            data: data,
                            success: function(response) {
                                lastInsertedId = response.recordsTotal + 1;
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                return row.caller.first_name + ' ' + row.caller
                                    .last_name;
                            },
                            orderable: false,
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                return row.caller.city + ' ' + row.caller.region + ' ' +
                                    row.caller.street + ' ' + row.caller.house_number +
                                    ' ' + row.caller.flat_number;
                            },
                            orderable: false,
                        },
                        {
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                //status == OPEN,DISPATCHED,APPROVED,DECLINED,DEPARTED,ARRIVED,COMPLETED,CANCELLED
                                let StatusStates = {
                                    'OPEN': 'primary',
                                    'DISPATCHED': 'warning',
                                    'APPROVED': 'success',
                                ...
                                let badgeStatus = StatusStates[status];


                                return `<span class="badge badge-${badgeStatus}">${StatusTranslates[row.status]}</span>`
                            },
                        },
                        {
                            data: 'staff',
                    ...
                    columnDefs: [
                        ...
                        // },
                        {
                            ...
                            className: 'text-end',
                            render: function(data, type, row) {
                                let setDisabledAssign =
                                    '<a data-bs-toggle="modal" data-bs-target="#kt_modal_new3_target" data-id="' +
                                    row.id +
                                    '" class="menu-link px-3 assign-worker">İcraçı təyin et</a>';
                                if (row.status != "OPEN" && row.status != "DECLINED") {
                                    setDisabledAssign =
                                        '<a data-bs-toggle="tooltip" data-bs-placement="top" title="İcraçı təyin edilib" class="menu-link px-3 disabled">İcraçı təyin et</a>';
                                }
                                return `
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-150px py-4" data-kt-menu="true">
                                @can('read-emergency')
                                    <div class="menu-item px-3">
                                        <a data-bs-toggle="modal" data-bs-target="#kt_modal_view_info" data-id="${row.id}" class="view-modal-btn menu-link px-3">Bax</a>
                                    </div>
                                @endcan
            ...
            var handleDeleteRows = () => {
                ...
                deleteButtons.forEach(d => {
                    ...
                    d.addEventListener('click', function(e) {
                        ...
                        Swal.fire({
                            text: "{{ __('admin::general.pages.emergencyCall.list.are_you_sure_you_want_to_delete_this') }} " +
                                customerName + "?",
                            icon: "warning",
                            ...
                            if (result.value) {
                                ...
                                let id = parent.querySelectorAll('td')[0]
                                    ...
                                    let url =
                                    `{{ route('admin.ajax.emergencycall.delete', ['id' => '-1']) }}`;
                                ...
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        Swal.fire({
                                            text: "" +
                                                customerName +
                                                " {{ __('admin::general.pages.emergencyCall.list.emergency_was_successfully_deleted') }}" +
                                                "!.",
                                            icon: "success",
                                            buttonsStyling: false,
                                    ...
                                    error: function(xhr) {
                                        Swal.fire({
                                            text: customerName +
                                                "{{ __('admin::general.pages.emergencyCall.list.emergency_was_not_deleted') }}" +
                                                xhr
                                                .responseJSON
                                                .message,
                                            icon: "error",
