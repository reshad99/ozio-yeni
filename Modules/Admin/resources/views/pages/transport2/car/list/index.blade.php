...
@section('content')
                    ...
                    <div class="d-flex flex-stack mb-5">
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" data-kt-docs-table-filter="search" class="form-control  w-250px ps-15"
                                placeholder="{{ __('admin::general.pages.transport.car.list.search_car') }}" />
                        </div>
                        <div class="d-flex justify-content-end" data-kt-docs-table-toolbar="base">
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                data-bs-target="#kt_modal_new_target">
                                <i class="ki-duotone ki-plus fs-2"></i>
                                {{ __('admin::general.pages.transport.car.list.add_new_car') }}
                            </button>
                        </div>
                    ...
                    <table style="border-radius: 10px; overflow: hidden" id="kt_datatable_example_1"
                        ...
                        <thead>
                            <tr class="text-start text-white bg-primary fw-bold fs-7 text-uppercase gs-0">
                                <th class="px-3 w-40px">{{ __('admin::general.pages.emergencyCall.list.id') }}</th>
                                <th>{{ __('admin::general.pages.transport.car.list.brand') }}</th>
                                <th>{{ __('admin::general.pages.transport.car.list.serial_number') }}</th>
                                <th>{{ __('admin::general.pages.transport.car.list.assignment') }}</th>
                                <th>{{ __('admin::general.pages.transport.car.list.department') }}</th>
                                <th>{{ __('admin::general.shared.created_at') }}</th>
    ...
    @include('admin::pages.transport2.car.list.sections.add-modal')
    @include('admin::pages.transport2.car.list.sections.edit-modal')
@endsection
...
@section('scripts')
    ...
    @include('admin::pages.transport2.car.list.scripts.datatable')
    @include('admin::pages.transport2.car.list.scripts.add-modal')
    @include('admin::pages.transport2.car.list.scripts.edit-modal')
    <script>
        ...
        window.loadAjax = async function() {
            if (firstTimeClick) {
                let promise = new Promise(function(resolve, reject) {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "{{ route('admin.ajax.department.list') }}", // Update with your API endpoint
                        success: function(response) {
                            //fill response data to all department_id named select2
                            var select2s = document.querySelectorAll('[name="department_id"]');
                            for (var i = 0; i < select2s.length; i++) {
                                ...
                                for (var j = 0; j < response.length; j++) {
                                    ...
                                    option.innerHTML = response[j].name;
