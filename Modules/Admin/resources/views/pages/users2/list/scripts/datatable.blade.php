            ...
            var initDatatable = function() {
                window.datatable = dt = $("#kt_datatable_example_1").DataTable({
                    ...
                    ajax: function(data, callback, settings) {
                        ...
                        data.with = ['roles', 'organisation', 'region', 'department'];
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                let url = "{{ route('admin.user.view', '-1') }}";
                                url = url.replace('-1', row.id);
                                let url2 = "{{ route('admin.user.view', '-1') }}";
                                url2 = url2.replace('-1', row.id);

                                return `<div class="d-flex align-items-center">
                                    <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                                        <a href="${url}">
                                            <div class="symbol-label fs-3 bg-light-danger text-danger">
                                                ${row.first_name[0].toUpperCase() + row.last_name[0].toUpperCase()}</div>
                                        </a>
                                    </div>
                                    <div class="d-flex flex-column">
                                        <a href="${url2}"
                                            class="text-gray-800 text-hover-primary mb-1">${row.first_name} ${row.last_name}</a>
                                        <span>${row.email}</span>
                                    </div>
                                </div>
                                `;
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                return row.organisation.org_name;
                            }
                        },
                        {
                            data: 'region',
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                if (row.roles.length > 0) {
                                    row.roles.forEach(r => {
                                        html +=
                                            `<div class="badge badge-light-green badge-inline">${RolesTranslates[r.name]}</div>`;
                                    });
                                } else {
                                    html +=
                    ...
                    d.addEventListener('click', function(e) {
                        ...
                        Swal.fire({
                            ...
                            if (result.value) {
                                ...
                                let url =
                                    `{{ route('admin.ajax.user.delete', ['id' => '-1']) }}`;
                                ...
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        Swal.fire({
                                            text: "{{ __('admin::general.shared.you_have_deleted') }}" +
                                                customerName +
                                                "!.",
                                            icon: "success",
                                            buttonsStyling: false,
                                    ...
                                    error: function(xhr) {
                                        Swal.fire({
                                            text: customerName +
                                                " {{ __('admin::general.shared.was_not_deleted') }}" +
                                                xhr
                                                .responseJSON
                                                .message,
                                            icon: "error",
