@can('update-organisation')
    <script>
        $(document).on('click', "a[data-bs-target='#kt_modal_new2_target']", async function() {
            ...
            let url =
                `{{ route('admin.ajax.organisation.read', ['id' => '-1']) }}`;
            url = url.replace("-1", org_id);
            //ajax read role data
            $.ajax({
                type: "GET",
                ...
                success: function(response) {
                    ...
                    form.find("input[name='org_name']").val(response.org_name);
                        ...
                        form, {
                            fields: {
                                name: {
                                    validators: {
                                        notEmpty: {
                                            message: "{{ __('admin::general.pages.organisation.list.department_region_code') }}"
                                        }
                                    }
                                },
                                region_code: {
                                    validators: {
                                        notEmpty: {
                                            message: "{{ __('admin::general.pages.organisation.list.department_region_code_is_required') }}"
                                        }
                                    }
                                },
                            },
                        ...
                        if (validator) {
                            validator.validate().then(function(status) {
                                ...
                                if (status == 'Valid') {
                                    ...
                                    let url =
                                        `{{ route('admin.ajax.organisation.update', ['id' => '-1']) }}`;
                                    console.log(url);

                                    let org_id = $("#kt_modal_new2_target_form")
                                        .find(
