...
class UpdateStoreRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'name' => ['required', 'string', 'max:25', Rule::unique('stores', 'name')->ignore($this->store, 'id')->whereNull('deleted_at')],
            'store_code' => ['required', 'string', 'max:25', Rule::unique('stores', 'store_code')->ignore($this->store, 'id')->whereNull('deleted_at')],
            'currency_id' => ['required', Rule::exists('currencies', 'id')->whereNull('deleted_at')],
            'phone' => ['required', 'string', 'max:25', Rule::unique('stores', 'phone')->ignore($this->store, 'id')->whereNull('deleted_at')],
            'city_id' => ['required', Rule::exists('cities', 'id')->whereNull('deleted_at')],
            'email' => ['sometimes', 'email', Rule::unique('stores', 'email')->ignore($this->store, 'id')->whereNull('deleted_at')],
            'password' => ['sometimes', 'string', Password::min(6), 'confirmed'],
            'device_id' => ['sometimes', 'string', 'max:25', Rule::unique('stores', 'device_id')->ignore($this->store, 'id')->whereNull('deleted_at')],
